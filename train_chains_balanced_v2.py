"""
Training script for the BALANCED experiment — v2.

Same as train_chains_balanced.py, plus the wrong-letter mask fix:
in unhealthy examples (any row whose output contains a STOP token),
the loss is additionally zeroed on the wrong-letter token — the first
token of the last block before STOP. This makes the supervision
symmetric with the peek-letter mask already applied to healthy partials
and removes the correct/wrong label collision at late decision slots.

The wrong-letter position is detected at runtime from input_ids — no
...
class LitChains(L.LightningModule):
    def __init__(self, cfg, model, preprocessor, train_batches,
        ...
        self.semi_token_id = semi_token_id
    ...
    def _mask_wrong_letter(self, input_ids, targets):
        # For every row containing a STOP token, zero the loss on the
        # wrong-letter token — the first token of the block preceding STOP.
        # Output layout for an unhealthy example:
        #   ... [TRACE] B0 ; B1 ; ... ; W_letter : <wrong trace> ; STOP [EOS]
        # So the wrong letter sits right after the `;` that precedes the
        # "; STOP" tail — i.e. the second-to-last `;` in the row (or the
        # [TRACE] delimiter itself if the mistake is at step 0, in which
        # case the wrong block is the very first block after [TRACE]).
        stop_mask = (input_ids == self.stop_token_id)
        has_stop = stop_mask.any(dim=1)
        rows = has_stop.nonzero(as_tuple=True)[0]
        if rows.numel() == 0:
            return targets
        ...
        for r in rows.tolist():
            row_ids = input_ids[r]
            # Position of STOP.
            sp = int(stop_mask[r].nonzero(as_tuple=True)[0][0])

            # The `;` immediately before STOP should be at sp - 1. From
            # there, walk left to find the previous `;` — the left boundary
            # of the wrong block. If none, fall back to the [TRACE]
            # delimiter (wrong block is the first block).
            left_boundary = -1
            for i in range(sp - 2, -1, -1):
                tid = int(row_ids[i])
                if tid == self.semi_token_id:
                    left_boundary = i
                    break
                if tid == self.delimiter_token_id:
                    left_boundary = i
                    break
            if left_boundary < 0:
                continue

            wrong_letter_pos = left_boundary + 1
            # Targets are shifted-next-token labels aligned with input_ids,
            # so masking targets[r, wrong_letter_pos] zeroes the loss on
            # "predict the wrong letter at this position".
            if 0 <= wrong_letter_pos < targets.shape[1]:
                targets[r, wrong_letter_pos] = -100
    ...
    def mask_targets(self, input_ids, target_ids, mask_last_n=None):
        ...
        # Peek-letter + [EOS] mask for healthy partials (unchanged from v1).
        ...
        # v2 addition: wrong-letter mask for unhealthy examples.
        targets = self._mask_wrong_letter(input_ids, targets)
...
def _single_token_id(tokenizer, piece, name):
    ids = tokenizer.encode(piece, add_special_tokens=False)
    assert len(ids) == 1 and ids[0] != tokenizer.unk_token_id, \
        f"{name} must be a single in-vocab token, got IDs: {ids}"
    return ids[0]
...
def main(cfg: DictConfig):
    ...
    stop_token_id = _single_token_id(tokenizer, "STOP", "STOP")
    semi_token_id = _single_token_id(tokenizer, ";", "';'")
    print(f"STOP token ID: {stop_token_id}  ;  ';' token ID: {semi_token_id}")

    lit_model = LitChains(
        cfg=cfg, model=model, train_batches=train_size,
        preprocessor=preprocessor, delimiter_token_id=trace_token_id,
        eos_token_id=eos_token_id,
        stop_token_id=stop_token_id, semi_token_id=semi_token_id,
    )
